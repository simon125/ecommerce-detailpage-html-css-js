...
$thumbnailsContainer.addEventListener("click", ({ target }) => {
  if (target.classList.contains("thumbnail")) {
    $productImage.src = `./assets/images/image-product-${target.id}.jpg`;
    document
...
$imageButtonPrev.addEventListener("click", () => {
  ...
  if (prevActiveThumbnail.id * 1 > 1) {
    $productImage.src = `./assets/images/image-product-${
      prevActiveThumbnail.id * 1 - 1
    }.jpg`;
